...
final class PetIconStatusPicker: UIView {
    ...
    private let pickerTitle = TextLabel(
        ...
        text: L10n.Pets.Form.IconPicker.title,
    ...
    private lazy var petStatusCollectionView: UICollectionView = {
        ...
        collection.register(
            PetIconStatusCollectionViewCell.self,
            forCellWithReuseIdentifier: PetIconStatusCollectionViewCell.identifier
        )
        return collection
    }()

    private let statuses = PetIconStatus.allCases
    private var selectedStatus = PetIconStatus.none
...
extension PetIconStatusPicker: UICollectionViewDataSource {
    ...
    func collectionView(
        ...
        if let cell = collectionView.dequeueReusableCell(
            withReuseIdentifier: PetIconStatusCollectionViewCell.identifier,
            for: indexPath
        ) as? PetIconStatusCollectionViewCell {
            let isSelectedStatus = statuses[indexPath.row] == selectedStatus
            cell.configure(
                statuses[indexPath.row],
